...
function getModelPickerActionBarActionProvider(commandService: ICommandService, chatEntitlementService: IChatEntitlementService, productService: IProductService): IActionProvider {
	...
	const actionProvider: IActionProvider = {
		getActions: () => {
			...
			// Add Ollama connection option
			additionalActions.push({
				id: 'connectOllama',
				label: localize('chat.connectOllama', "Connect to Ollama"),
				enabled: true,
				tooltip: localize('chat.connectOllama.tooltip', "Connect to local Ollama models"),
				class: undefined,
				run: () => {
					commandService.executeCommand('workbench.action.chat.connectOllama');
				}
			});

			// Add refresh Ollama models option
			additionalActions.push({
				id: 'refreshOllama',
				label: localize('chat.refreshOllama', "Refresh Ollama Models"),
				enabled: true,
				tooltip: localize('chat.refreshOllama.tooltip', "Refresh the list of Ollama models"),
				class: undefined,
				run: () => {
					commandService.executeCommand('workbench.action.chat.refreshOllamaModels');
				}
			});

			// Commented out sign-in/upgrade options - using Ollama instead
			...
			// 		label: isNewOrAnonymousUser ? localize('chat.moreModels', "Хэлний загвар сонгох") : localize('chat.morePremiumModels', "Add Premium Models"),
			// 		enabled: true,
			// 		// allow-any-unicode-next-line
			// 		tooltip: isNewOrAnonymousUser ? localize('chat.moreModels.tooltip', "Хэлний загвар сонгох") : localize('chat.morePremiumModels.tooltip', "Add Premium Models"),
			// 		class: undefined,
			// 		run: () => {
